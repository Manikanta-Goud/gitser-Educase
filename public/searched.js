document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderProfiles(profiles) {
        if (profiles.length === 0) {
            grid.innerHTML = '<p class="empty-state">No profiles found.</p>';
            return;
        }
            ...
            return `
                <div class="history-card" style="display:block; position:relative;" id="card-${p.username}">
                    ...
                    <div class="card-actions">
                        <button class="btn-delete" data-username="${p.username}">Delete Profile</button>
                    </div>
                </div>
            `;
        }).join('');
        ...
        document.querySelectorAll('.btn-delete').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                if (!confirm(`Are you sure you want to delete the profile for @${uname}?`)) return;
